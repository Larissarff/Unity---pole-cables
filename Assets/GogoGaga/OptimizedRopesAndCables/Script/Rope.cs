        ...
        private void SetSplinePoint()
{
    ...
    // Definição das variáveis físicas
    float span = Vector3.Distance(startPoint.position, endPoint.position);
    float cableWeight = 1.80f; // Exemplo: peso do cabo (ajuste conforme necessário)
    float tension = 2200f; // Exemplo: esforço do cabo (ajuste conforme necessário)

    // Cálculo da flecha máxima (midPointPosition)
    float midPointPosition = (span * span * cableWeight) / (8 * tension);

    // Definir o ponto médio da curva no eixo X e Z, aplicando a flecha no eixo Y
    Vector3 mid = (startPoint.position + endPoint.position) / 2;
    mid.y -= midPointPosition; // Ajusta a posição do meio considerando a flecha

    // Atualiza o valor do midPoint no Bezier
    targetValue = mid;
    mid = currentValue;
